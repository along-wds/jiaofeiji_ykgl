...
void Jmsh1::on_pushButton_func3_clicked()
{
    ...
    QPushButton *btn6=new QPushButton(ui->frame);
    btn6->setObjectName(QStringLiteral("pushButton_6"));
    btn6->setMinimumSize(QSize(178, 235));
    btn6->setMaximumSize(QSize(178, 235));
    horizontalLayout->addWidget(btn6);
    btn6->setStyleSheet("QPushButton{border-image: url(:/image/picture/ykgl/1-6.png);}");
    //horizontalLayout->set
    connect(btn1,SIGNAL(clicked()),this,SLOT(openScan()));
}

void Jmsh1::on_pushButton_5_clicked()
{
    disconnectSlots();
    disconnect(socket->accessManager, SIGNAL(finished(QNetworkReply*)), this, SLOT(replyFinished(QNetworkReply*)));
