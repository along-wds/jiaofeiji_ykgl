    ...
    ui(new Ui::Ydmx)
{
    ...
    ui->frame_3->setStyleSheet("QFrame#frame3{background-color:#dfdfdf;border:1px;border-radius:2px;}");
   ...
   setCalendarStyle();
    ...
    startTimer();
    QString string;
    OperateFile::readiniFile("INTERFACE.DATA","interface/GETYDMX",string);

    socket->HttpSend(QUrl(QString(string).arg(socket->message.yhdabh).arg(ui->dateEdit_3->date().toString("yyyy-MM")).arg(ui->dateEdit_2->date().toString("yyyy-MM"))));
    OperateFile::tracelog(QString(string).arg(socket->message.yhdabh).arg(ui->dateEdit_3->date().toString("yyyy-MM")).arg(ui->dateEdit_2->date().toString("yyyy-MM")));
...
void Ydmx::on_pushButton_clicked()
{
    startTimer();
    QString string;
    OperateFile::readiniFile("INTERFACE.DATA","interface/GETYDMX",string);
    socket->HttpSend(QUrl(QString(string).arg(socket->message.yhdabh).arg(ui->dateEdit_3->date().toString("yyyy-MM")).arg(ui->dateEdit_2->date().toString("yyyy-MM"))));
    //qDebug()<<QString(string).arg(socket->client_msg.yhdabh).arg(ui->dateEdit_2->text()).arg(ui->dateEdit_3->text());
}
void Ydmx::replyFinished(QNetworkReply * reply)
{
    ...
    if(reply->error() == QNetworkReply::NoError)
     {
          ...
          else //未查到
          {
              ...
              messagebox->displayWithoutButton(this,"查询失败");
          }
...
void Ydmx::setList()
{
  ...
  ui->tableWidget->setColumnCount(9);   //设置列数为9
  ui->tableWidget->setEditTriggers(QAbstractItemView::NoEditTriggers);
  ui->tableWidget->setSelectionBehavior(QAbstractItemView::SelectRows);
  QStringList header;
  header<<"发行年月"<<"电价名称"<<"计量表号"<<"倍率"<<"上次读数"<<"本次读数"<<"电量"<<"电价"<<"电费(元)";
...
void Ydmx::on_pushButton_backHomePage_3_clicked()
{
    ...
    socket->effect->begin(this,OperateFile::ui_lookup,RIGHTTOLEFT,NONE,CLOSE,NEIYE);
    OperateFile::ui_lookup->init();
}
void Ydmx::waitTimeout()
{
    emit updateDispaly();
    ui->lcdNumber->show();
    ui->label_6->show();
}
void Ydmx::mousePressEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
    m_currenttime=SEC;
...
bool Ydmx::eventFilter(QObject *watched, QEvent *event)
{
...
void Ydmx::on_pushButton_2_clicked()
{
        startTimer();
        flushPage(-2);
}

void Ydmx::on_pushButton_3_clicked()
{
        startTimer();
        flushPage(0);
}

void Ydmx::on_pushButton_home_clicked()
{
...
void Ydmx::on_pushButton_purchase_clicked()
{
...
void Ydmx::on_pushButton_search_clicked()
{
    ui->pushButton_search->setEnabled(false);
